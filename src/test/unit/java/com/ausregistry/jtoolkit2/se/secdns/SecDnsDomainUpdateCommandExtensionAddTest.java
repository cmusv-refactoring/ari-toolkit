...
public class SecDnsDomainUpdateCommandExtensionAddTest {
    ...
    private static class MockInjector implements OngoingInjecter {
        private final Object mock;

        MockInjector(Object mock) {
            this.mock = mock;
        }
    ...
    public void testSecDNSKeyDataNotAllowedWithDSData() throws Exception {
        ...
        try {
            cmd.toXML();
            Assert.fail("Should trigger a SAXParserException");
    ...
    @Test
    public void testMockInjector() throws Exception {
        final Command cmd = new DomainUpdateCommand("jtkutest.com.au", "jtkUT3st", null, null, "JTKCON");
        ...
        MockInjector keyDataInjecter = new MockInjector(kd);
        Object injectedKeyData = keyDataInjecter.thenInject();
        Assert.assertNotNull("KeyData should be successfully injected", injectedKeyData);
        Assert.assertEquals("Injected KeyData should match the original", kd, injectedKeyData);

        MockInjector dsDataInjecter = new MockInjector(dsData);
        Object injectedDsData = dsDataInjecter.thenInject();
        Assert.assertNotNull("DSData should be successfully injected", injectedDsData);
        Assert.assertEquals("Injected DSData should match the original", dsData, injectedDsData);

        DSOrKeyType addData = new DSOrKeyType();
        addData.addToKeyData((KeyData) injectedKeyData);
        addData.addToDsData((DSData) injectedDsData);
        ext.setAddData(addData);
        ...
        try {
            ...
            System.out.println("Caught SAXParserException: " + spe.getMessage());
            Assert.assertTrue("Should trigger a SAXParserException with message about keyData",
                    spe.getMessage().contains("element 'keyData'"));
        }
    }
